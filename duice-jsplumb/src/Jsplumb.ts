...
namespace duice.plugin {
    ...
    export class Jsplumb extends duice.CustomElement<object> {
        ...
        constructor(htmlElement: HTMLElement, bindData: object, context: object) {
            ...
            this.container.classList.add(getNamespace() + '-diagram-container');
        ...
        createStyle(): HTMLStyleElement {
            ...
            style.innerHTML = `
                .${getNamespace()}-diagram-container {
                    background-size: 1rem 1rem;
                    background-image: radial-gradient(circle, rgba(128, 128, 128, 0.25) 1px, rgba(0, 0, 0, 0) 1px);
                }
                .${getNamespace()}-diagram-element {
                }
                .${getNamespace()}-diagram-endpoint-source {
                    cursor: pointer;
                    opacity: 0.5;
                }
                .${getNamespace()}-diagram-endpoint-source-hover {
                    opacity: 1.0;
                }
                .${getNamespace()}-diagram-endpoint-target {
                    opacity: 0.5;
                }
                .${getNamespace()}-diagram-endpoint-target-hover {
                    opacity: 1.0;
                }
                .${getNamespace()}-diagram-connector-disconnect {
                    display: block;
                    cursor: pointer;
                ...
                .${getNamespace()}-diagram-connector-disconnect:hover {
                    opacity: 1;
                }
            `;
            return style;
        ...
        fitContainerToContent(): void {
            let maxWidth = 0;
            let maxHeight = 0;
            this.container.querySelectorAll(`.${getNamespace()}-diagram-element`)
                .forEach(element => {
        ...
        createElementItem(elementObject: object, context: object, index: number): void {
            ...
            elementItem.classList.add(getNamespace() + '-diagram-element');
            ...
            let sourceEndpoint = this.jsPlumbInstance.addEndpoint(elementItem, {
                ...
                endpoint:['Rectangle',{ width:10, height:10, cssClass:`${getNamespace()}-diagram-endpoint-source`, hoverClass:`${getNamespace()}-diagram-endpoint-source-hover`}],
            ...
            let targetEndpoint = this.jsPlumbInstance.addEndpoint(elementItem, {
                ...
                endpoint:['Dot',{ radius:6, cssClass:`${getNamespace()}-diagram-endpoint-target`, hoverClass:`${getNamespace()}-diagram-endpoint-target-hover`}],
        ...
        createConnectorItem(connectorObject: object): void {
            ...
            let connectorItem = this.jsPlumbInstance.connect({
                ...
                overlays: [
                    ...
                    ['Label', {
                        label: `<span class="${getNamespace() + '-diagram-connector-disconnect'}"></span>`,
                        location: 0.5,
                        events: {
                            click: (labelOverlay, originalEvent) => {
                                if(this.isEditable()) {
